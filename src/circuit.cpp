...
vector<Gate*> *Circuit::mru_topological_ordering(std::map<Qubit*, int> qubit_map, Machine *M){
	std::vector<Gate*> ready_gates;
	...
	for(auto Gi : gates){
		...
		if(in_degree[Gi] == 0){
			ready_gates.push_back(Gi);
		}
	}
	assert(!ready_gates.empty());

	//initialize MRU trackers - one for each region
	vector<int> mru_tracker;
	for(int i=0; i<M->num_partitions; i++){
		mru_tracker.push_back(0);
	}

	int cx_id = 1;
	//kill edges, pick vertices in mru order
	while(!ready_gates.empty()){
		Gate *best_gate = ready_gates[0];
		int best_score = 0;
		for(auto Gi : ready_gates){
			if(Gi->nvars == 1){
				best_gate = Gi;
				break; //schedule one qubit gates without any updates to mru
			}else if(Gi->nvars == 2){
				int qid1, qid2;
				qid1 = qubit_map[Gi->vars[0]];
				qid2 = qubit_map[Gi->vars[1]];
				int r1 = M->qubits[qid1]->partition;
				int r2 = M->qubits[qid2]->partition;
				int my_score = mru_tracker[r1];
				if(mru_tracker[r2] > my_score) my_score = mru_tracker[r2];
		...
		//update MRU trackers if we are scheduling a two qubit gate
		if(best_gate->nvars == 2){
			int qid1, qid2;
			qid1 = qubit_map[best_gate->vars[0]];
			qid2 = qubit_map[best_gate->vars[1]];
			int r1 = M->qubits[qid1]->partition;
			int r2 = M->qubits[qid2]->partition;
			mru_tracker[r1] = cx_id;
			mru_tracker[r2] = cx_id;
			cx_id++;
		}
		ans->push_back(best_gate);
		ready_gates.erase(remove(ready_gates.begin(), ready_gates.end(), best_gate), ready_gates.end());
		for(auto s : best_gate->succ){
			in_degree[s]--;
			if(in_degree[s] == 0) ready_gates.push_back(s);
		}
	}
